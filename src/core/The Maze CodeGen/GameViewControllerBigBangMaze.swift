...
@available(iOS 13.0, *)
class GameViewControllerBigBangMaze: UIViewController {
...
//        self.view = UIView(frame: UIScreen.main.bounds)
  ...
  let EnSoString = "MetricsBigBang"
  var InitMetrics: [NSManagedObject] = []
  
  override func viewDidAppear(_ animated: Bool) {
    saveMode(mode: "MetricsBigBang", modeName: "Big Bang")
    loadInitMetrics()
  ...
  override func viewDidLoad() {
    ...
    //    print(view)
      ...
      let scene = GameSceneBigBangMaze(size: temp)
    ...
    self.view.addSubview(swipingButton)
      ...
      /* if scene.isSolved()==1 {
       let scene=GameSceneBigBangMaze(size: view.bounds.size)
