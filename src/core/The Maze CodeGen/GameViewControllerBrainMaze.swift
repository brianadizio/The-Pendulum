...
@available(iOS 13.0, *)
class GameViewControllerBrainMaze: UIViewController {
...
//        self.view = UIView(frame: UIScreen.main.bounds)
  ...
  let EnSoString = "MetricsBrain"
  var InitMetrics: [NSManagedObject] = []
  
  override func viewDidAppear(_ animated: Bool) {
    saveMode(mode: "MetricsBrain", modeName: "Brain")
    loadInitMetrics()
  ...
  override func viewDidLoad() {
    ...
    //    print(view)
      ...
      let scene = GameSceneBrainMaze(size: temp)
    ...
    self.view.addSubview(swipingButton)
      ...
      /* if scene.isSolved()==1 {
       let scene=GameSceneBrainMaze(size: view.bounds.size)
