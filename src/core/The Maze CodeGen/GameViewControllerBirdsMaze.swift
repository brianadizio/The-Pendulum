...
@available(iOS 13.0, *)
class GameViewControllerBirdsMaze: UIViewController {
...
//        self.view = UIView(frame: UIScreen.main.bounds)
  ...
  let EnSoString = "MetricsBirds"
  var InitMetrics: [NSManagedObject] = []
  
  override func viewDidAppear(_ animated: Bool) {
    saveMode(mode: "MetricsBirds", modeName: "Birds")
    loadInitMetrics()
  ...
  override func viewDidLoad() {
    ...
    //    print(view)
      ...
      let scene = GameSceneBirdsMaze(size: temp)
    ...
    self.view.addSubview(swipingButton)
      ...
      /* if scene.isSolved()==1 {
       let scene=GameSceneBirdsMaze(size: view.bounds.size)
